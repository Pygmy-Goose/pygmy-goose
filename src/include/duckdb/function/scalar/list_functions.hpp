...
struct ListPositionFun {
	...
	static constexpr const char *Parameters = "list,element";
	static constexpr const char *Description = "Returns the index of the `element` if the `list` contains the "
	                                           "`element`. If the `element` is not found, it returns `NULL`.";
...
struct ListZipFun {
	...
	static constexpr const char *Description =
	    "Zips n `LIST`s to a new `LIST` whose length will be that of the longest list. Its elements are structs of n "
	    "elements from each list `list_1`, …, `list_n`, missing elements are replaced with `NULL`. If `truncate` is "
	    "set, all lists are truncated to the smallest list length.";
	static constexpr const char *Example = "list_zip([1, 2], [3, 4], [5, 6])\002list_zip([1, 2], [3, 4], [5, 6, "
	                                       "7])\002list_zip([1, 2], [3, 4], [5, 6, 7], true)";
	static constexpr const char *Categories = "list";
...
struct ListResizeFun {
	...
	static constexpr const char *Description = "Resizes the `list` to contain `size` elements. Initializes new "
	                                           "elements with `value` or `NULL` if `value` is not set.";
...
struct ArrayExtractFun {
	...
	static constexpr const char *Parameters = "string::VARCHAR,index::ANY\001struct::STRUCT,entry::VARCHAR\001struct::"
	                                          "STRUCT,index::ANY\001list::ANY[],index::ANY";
	static constexpr const char *Description =
	    "Extracts a single character from a `string` using a (1-based) `index`.\001Extracts the named `entry` from the "
	    "`STRUCT`.\001Extracts the entry from an unnamed `STRUCT` (tuple) using an index (1-based).\001Extracts the "
	    "`index`th (1-based) value from the `list`.";
	static constexpr const char *Example = "array_extract('DuckDB', 2)\001array_extract({'i': 3, 'v2': 3, 'v3': 0}, "
	                                       "'i')\001array_extract(row(42, 84), 1)\001array_extract([4, 5, 6], 3)";
